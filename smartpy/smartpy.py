...
def open_chest(chest_key, chest, time):
    return Expr("open_chest", [spExpr(chest_key), spExpr(chest), spExpr(time), get_file_line_no()])
...
def spMetaExpr(x, context = "expression"):
    ...
    if isinstance(x, OnOffchainView):
        return x
...
TChest_key = TSimple("chest_key") # Represents the decryption key, alongside with a proof that the key is correct.
TChest = TSimple("chest") # Represents timelocked arbitrary bytes with the necessary public parameters to open it.
...
def resolve(value):
    return Expr("resolve", [spExpr(value), get_file_line_no()])
...
class TestAccount:
    def __init__(self, seed):
        ...
        self.address = resolve(self.e.address)
        self.public_key_hash = resolve(self.e.public_key_hash)
        self.public_key = resolve(self.e.public_key)
        self.secret_key = resolve(self.e.secret_key)
...
class build_set(WouldBeValue):
    def __init__(self, l = None):
        if l is None:
            l = []
        if isinstance(l, pySet) and any(isinstance(y, Expr) for y in l):
            raise Exception(
                "{e1, ..., en} syntax is forbidden for SmartPy Expr. Please use sp.set([e1, .., en])"
            )
        self.l = l
...
class Contract:
    ...
    def onoffchain_view(self, ov):
        self.init_internal()
        self.views.append(ov)
        class View:
            def __init__(self, ov, contract_id):
                self.ov = ov
                self.contract_id = contract_id
            def __call__(self, params):
                return Expr("view", [self.ov.name, contract_address(self.contract_id), spExpr(params), sp.types.conv(None), get_file_line_no()]).open_some("View %s is invalid!" % self.ov.name)
        return View(ov, self.contractId)
    ...
    def addMessage(self, addedMessage):
        ...
        if nargs == 0:
            raise Exception("Entry point '%s' is missing a self parameter (line %s)." % (addedMessage.name, addedMessage.line_no.export()))
        elif nargs == 1:
        ...
        if x is not None:
            raise Exception(
                "Entry point failure for %s (line %s): entry points cannot have return statements."
                % (addedMessage.name, addedMessage.line_no.export())
            )
        self.mb = None
        sp.setMB(None)
    ...
    def collectMessages(self):
        ...
        for f in dir(self):
            ...
            if isinstance(attr, OnOffchainView):
                attr._l = self.onoffchain_view(attr)
    ...
    def export(self):
        ...
        for message in self.messages.values():
            if message.addedMessage.name is not None:
                ...
                if name in messages:
                    raise Exception("Two entry points are called %s (line %s)" % (name, message.addedMessage.line_no.export()))
        ...
        result = "(template_id %s \nstorage %s\nstorage_type (%s)\nmessages (%s)\nflags (%s)\nglobals (%s)\nviews (%s)\nentry_points_layout %s\ninitial_metadata (%s)\nbalance %s)" % (
            ...
            ( " ".join(ov.exportDef(self) for ov in self.views)
            ),
    ...
    def scenario_message(self, accept_unknown_types, address=None):
        self.collectMessages()
        data = {}
        ...
        if address is not None:
            data["address"] = spExpr(address).export()
        return data
...
class OnOffchainView:
    def __init__(self, name, f, pure, doc, lineNo, kind):
        self.name = name
        self.f = f
    ...
    def export(self):
        return (Expr("meta_view", [self.name, self.line_no])).export()
    ...
    def exportDef(self, contract):
        ...
        return "(%s %s %s %s %s %s %s)" % (self.kind, self.name, nargs > 1, self.line_no.export(), self.pure, json.dumps(self.doc), commands)
...
def offchain_view(pure = None, doc = None):
    def apply_offchain_view(f, name=None):
        if name is None:
            name = f.__name__
        return OnOffchainView(name, f, pure, doc, get_file_line_no(), "offchain")
    return apply_offchain_view

def onchain_view(pure = None, doc = None):
    def apply_onchain_view(f, name=None):
        if name is None:
            name = f.__name__
        return OnOffchainView(name, f, pure, doc, get_file_line_no(), "onchain")
    return apply_onchain_view
...
def constant(value, t = None):
    ...
    return Expr("constant", [value, sp.types.conv(t), get_file_line_no()])
...
def view(name, address, param, t = None, line_no = None):
    t = sp.types.conv(t)
    return Expr(
        "view", [name, spExpr(address), spExpr(param), sp.types.conv(t), get_file_line_no(line_no)]
    )
...
class Scenario:
    ...
    def register(self, element, show=False, accept_unknown_types=False, address=None):
        if isinstance(element, Contract):
            self.acc(element.scenario_message(accept_unknown_types, address), show)
    ...
    def prepare_constant_value(self, value, hash = None):
        ...
        data["line_no"] = get_file_line_no().export()
        self.messages += [data]
        return Expr("constant_scenario_var", [id, get_file_line_no()])
...
def add_simulation_target(contract, name="Simulation", shortname=None, profile=False, is_default=False, flags=[], constants=[]):
    def test():
...
def catch_exception(expression, t = None):
    return Expr("catch_exception", [spExpr(expression), sp.types.conv(t), get_file_line_no()])
...
def contract_baker(contractId):
    ...
    return resolve(Expr("contract_baker", [contractId, get_file_line_no()]))
...
class utils:
    ...
    def seconds_of_timestamp(_timestamp):
        """Convert timestamp to nat"""
        return abs(_timestamp - timestamp(0))
